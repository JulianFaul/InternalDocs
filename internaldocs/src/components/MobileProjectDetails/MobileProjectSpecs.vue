   ...
   <v-list  v-if="isEmpty" class="document-list__group">
        ...
        <v-list-tile  v-if="isEmpty" class="document-list__item">
            <v-list-tile-content>
                ...
                <v-list-tile-title ><span v-for="(usedBy, index) in loadedAppSpecs.usedBy" :key='index'> {{usedBy}}</span></v-list-tile-title>
            </v-list-tile-content>
            <v-spacer></v-spacer>
        </v-list-tile>
        ...
        <v-list-tile  v-if="isEmpty" class="document-list__item">
            <v-list-tile-content>
                ...
                <v-list-tile-title ><span v-for="(device, index) in loadedAppSpecs.devices" :key='index'> {{device}}</span></v-list-tile-title>
            </v-list-tile-content>
            <v-spacer></v-spacer>
        </v-list-tile>
        ...
        <v-list-tile  v-if="isEmpty" class="document-list__item">
            <v-list-tile-content>
                ...
                <v-list-tile-title ><span v-for="(store, index) in loadedAppSpecs.stores" :key='index'> {{store}}</span></v-list-tile-title>
            </v-list-tile-content>
            <v-spacer></v-spacer>
        </v-list-tile>
        ...
        <!-- -->
        <!-- Have to put in all the other information for myspecs i have only done myquotes -->
    ...
    <v-list v-if="isEmpty" class="document-list__group" v-for="(document_group,group_name,index) in groupedDocuments" :key="index">
           <v-subheader class="document-list__header"><h3>{{group_name}}</h3></v-subheader>
            ...
            <v-list-tile class="document-list__item" v-for="doc in groupedDocuments[group_name]" :key="doc.id">
                ...
                <v-spacer></v-spacer>
                  <v-icon  class="button">vertical_align_bottom</v-icon> 
                  <button class="button button__blue">Update</button> 
                  <v-icon class="button button__red-text">delete</v-icon> 
          </v-list-tile>
      </v-list>
      <!-- {{loadedAppSpecs}}-->
</div>
...
export default {
  ...
  methods:{
    ...
    downloadPriceList(path){
      console.log(this.loadedAppSpecs)
    }
  },
  computed: {
    isEmpty() {
